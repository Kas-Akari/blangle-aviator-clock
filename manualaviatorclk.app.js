...
const APP_NAME = 'manualaviatorclk';
...
function updateAVWX() {
  ...
  if (AVWXrequest) return;  // already updating
  ...
  let icao = (settings.icao || "").toUpperCase().trim();
  if (!/^[A-Z0-9]{4}$/.test(icao)) {
    METAR = 'ERR: Invalid ICAO in settings';
    METARlinesCount = 0;
    METARscollLines = 0;
    METARts = undefined;
    showUpdateAVWXstatus('X');
  ...
  // get latest METAR from configured airport (via AVWX API)
  AVWXrequest = avwx.request('metar/'+ icao, '', data => {
  if (avwxTimeout) clearTimeout(avwxTimeout);
  avwxTimeout = undefined;
